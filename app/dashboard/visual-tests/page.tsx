...
export default function VisualTestsPage() {
  const [items, setItems] = useState<VisualTestsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/visual-tests')
      .then(r => r.json())
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/visual-tests', {
      method: 'POST',
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/visual-tests/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Visual Regression Tests</h1>
        <button
