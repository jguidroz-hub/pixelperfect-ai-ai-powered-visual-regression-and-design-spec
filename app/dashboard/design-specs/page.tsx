...
export default function DesignSpecsPage() {
  const [items, setItems] = useState<DesignSpecsItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/design-specs')
      .then(r => r.json())
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/design-specs/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Design Specifications</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">Manage and upload design references</p>
